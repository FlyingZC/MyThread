...
public class T07ToastOMaticQueue
{
    ...
    public void run()
    {
        try
        {
            while (!Thread.interrupted())
            {
                ...
                Toast t = dryQueue.take();// 从生产吐司的队列中取吐司.若队列中的所有元素都take完了,这行代码会阻塞.直到队列中又加入了新的元素
                t.butter();// 然后给吐司加黄油
                System.out.println(t);
                butteredQueue.put(t);// 然后放入 [加过黄油的]队列
            }
        }
        catch (InterruptedException e)
        {
...
// Apply jam to buttered toast:在抹过黄油的吐司上涂果酱
